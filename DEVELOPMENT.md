...
The release workflow imports a code-signing cert into a temporary keychain
on the macOS runner and uses it to sign both the `.appex` and the host
bundle. Three repository secrets are required (already configured in
`jowtron/md-viewer`):

| Secret | Contents |
|--------|----------|
| `MACOS_CERT_P12_BASE64` | base64-encoded `.p12` exported from `login.keychain` containing the Apple Development cert + private key |
| `MACOS_CERT_PASSWORD` | the export password used when the `.p12` was created |
| `MACOS_CODESIGN_IDENTITY` | identity string passed to `codesign`, e.g. `Apple Development: you@example.com (TEAMID)` |

If `MACOS_CERT_P12_BASE64` is empty (e.g. on a fork), the import step exits
early and the workflow falls back to ad-hoc signing — bundles still build
and upload, but the QuickLook extension won't load on downloaders' Macs.

To rotate the cert (Apple Development certs expire after ~1 year):

```sh
PW=$(openssl rand -base64 32 | tr -d '\n=')
P12=$(mktemp -t apple_dev).p12
security export -k ~/Library/Keychains/login.keychain-db -t identities \
  -f pkcs12 -P "$PW" -o "$P12"
base64 -i "$P12" | gh secret set MACOS_CERT_P12_BASE64 --repo jowtron/md-viewer
printf '%s' "$PW" | gh secret set MACOS_CERT_PASSWORD --repo jowtron/md-viewer
# Update MACOS_CODESIGN_IDENTITY only if the cert's CN changed
rm -f "$P12"
```

The runner's keychain is created fresh each job and discarded when the job
ends; the `.p12` is decoded into `$RUNNER_TEMP`, imported, then deleted
